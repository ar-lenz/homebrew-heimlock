...
name: computer-use
description: >-
  Use Heimlock's computer-use CLI to inspect and operate local desktop app windows
  ...
  accessibility actions. Also use for browser windows, webviews, Heimlock app UI,
  or other desktop UI. Triggers include "computer use", "heimlock computer", "read
...
Use this skill for desktop UI through `heimlock computer`. When the requested target is a website or web app, operate the desktop browser app/window that contains the page.

## Preconditions

- Prefer `heimlock computer ...`; on Linux, use `heimlock-ide computer ...` if `heimlock` is unavailable. In this Heimlock worktree, use `./config/scripts/heimlock-dev computer ...` only when testing the local dev runtime.
...
heimlock computer capabilities --json
```

## Core Loop

```bash
heimlock computer list-apps --json
heimlock computer get-app-state --app com.spotify.client --json
heimlock computer click --app com.spotify.client --element-index 42 --json
```
...
```bash
heimlock computer get-app-state --app com.microsoft.edgemac --json
heimlock computer get-app-state --app Spotify --json
heimlock computer get-app-state --app pid:12345 --json
```
...
```bash
heimlock computer permissions --json
heimlock computer capabilities --json
heimlock computer list-apps --json
heimlock computer list-windows --app <app> --json
heimlock computer get-app-state --app <app> --json
heimlock computer get-app-state --app <app> --restore-window --json
heimlock computer click --app <app> --element-index <index> --json
heimlock computer click --app <app> --x 100 --y 100 --json
heimlock computer perform-secondary-action --app <app> --element-index <index> --action <name> --json
heimlock computer set-value --app <app> --element-index <index> --value "text" --json
heimlock computer type-text --app <app> --text "text" --json
heimlock computer press-key --app <app> --key Return --json
heimlock computer hotkey --app <app> --key CmdOrCtrl+A --json
heimlock computer paste-text --app <app> --text "text" --json
heimlock computer scroll --app <app> (--element-index <index> | --x <x> --y <y>) --direction down --json
heimlock computer drag --app <app> --from-element-index <index> --to-element-index <index> --json
heimlock computer drag --app <app> --from-x 100 --from-y 100 --to-x 300 --to-y 300 --json
```
...
```bash
printf '%s' "$TEXT" | heimlock computer set-value --app <app> --element-index <index> --value-stdin --json
```
...
```bash
heimlock computer get-app-state --app com.microsoft.edgemac --restore-window --json
heimlock computer set-value --app com.microsoft.edgemac --element-index <addressBarIndex> --value "test123" --json
heimlock computer press-key --app com.microsoft.edgemac --key Return --json
```
...
- `app_not_found`: run `list-apps` and retry with the bundle ID. If the target is a web app such as Gmail, choose the desktop browser app/window that contains it; do not retry `heimlock computer ... --app Gmail` unchanged because `heimlock computer` app selectors refer to desktop apps, not website names.
...
- `screenshot_failed`: use `--no-screenshot` if tree state is enough; if the message names Screen Recording or screenshots permission, run `heimlock computer permissions --id screenshots --json`.
- `accessibility_error`: run `heimlock computer capabilities --json`; if the message names Accessibility permission, run `heimlock computer permissions --id accessibility --json`.
- Empty tree or no screenshot: app may have no visible window, be minimized, or need permissions.
- Permission errors: run `heimlock computer permissions --json`, or `heimlock computer permissions --id accessibility --json` / `--id screenshots --json` when the message names one permission, use the setup UI, then retry.

## Next Action

Confirm Heimlock status unless already checked, then run `heimlock computer capabilities --json`. For website or web-app targets such as Gmail, identify the desktop browser app/window that contains the page, then get that target app state with `heimlock computer get-app-state --app <app> --json`.
